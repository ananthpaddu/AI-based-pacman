...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        value, action = self.max_value(gameState, 0, 0, self.a, self.b)
        #time.sleep(0.5)
        #util.raiseNotDefined()
        return action
    ...
    def value(self, gameState, agent, cur_depth, a, b):
        ...
        if (cur_depth == self.depth) | gameState.isWin() | gameState.isLose():
            return self.evaluationFunction(gameState)
        if agent == 0:
            val, action = self.max_value(gameState, agent, cur_depth, a, b)
            return val
        else:
            val, action = self.min_value(gameState, agent, cur_depth, a, b)
            return val
...
def betterEvaluationFunction(currentGameState):
    ...
    evalValue = (1/(totalFoodLeft + 1)) + (1/(min(foodDistances) + 2)) 
    distanceFromGhost = [manhattanDistance(newPos, position) for position in newGhostPositions]
    #evalValue = -1 if ((min(distanceFromGhost) == 1) & (newScaredTimes > 0)) else evalValue 
    #evalValue = -2 if ((min(distanceFromGhost) < 1) & (newScaredTimes > 0)) else evalValue 
    if (min(distanceFromGhost) < 2):
        if (newScaredTimes == 0):
            if min(distanceFromGhost) == 1:
                evalValue = -1
            else:
                evalValue = -2
    ...
    #print(foodDistances)
    #time.sleep(.05)
    #print('our score',evalValue)
    #print(newGhostPositions)
    #print(distanceFromGhost)
    #print(currentGameState.getScore())
    #print(newScaredTimes)
    return (evalValue + currentGameState.getScore())    
