...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        chosenIndex = self.alphabeta(gameState, 0, self.a, self.b, 0)
        legalMoves = gameState.getLegalActions(0)
        time.sleep(2)
        #util.raiseNotDefined()
        return legalMoves[chosenIndex]
    
    def alphabeta(self, gameState, cur_depth, a, b, agent):
        ...
        if agent == 0:
            ...
            counter = 0
            for nextState in nextPossibleStates:
                val = max(val, self.alphabeta(nextState, cur_depth, a, b, nextAgent))
                a = max(a, val)
                if a <= val:
                    best_action = counter
                if a >= b:
            ...
            val = 10000
            for nextState in nextPossibleStates:
                val = min(val, self.alphabeta(nextState, cur_depth, a, b, nextAgent))
                b = min(b, val)
                if a >= b:
                    break
            return val
