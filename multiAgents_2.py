...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def min_value(self, gameState, agent, cur_depth):
        ...
        for nextState in nextPossibleStates:
            if nextAgent == 0:
                min_val = min(min_val, self.max_value(nextState, nextAgent, cur_depth + 1))
            else:
                min_val = min(min_val, self.min_value(nextState, nextAgent, cur_depth + 1))
                
        print(nextAgent, min_val, cur_depth)
        return min_val
    ...
    def max_value(self, gameState, agent, cur_depth):
        ...
        nextAgent = (agent + 1) % gameState.getNumAgents()
        for nextState in nextPossibleStates:
            max_val = max(max_val, self.min_value(nextState, nextAgent, cur_depth + 1))
            print('pac values', max_val)    
        print('final decision', nextAgent, max_val, cur_depth)
        return max_val
